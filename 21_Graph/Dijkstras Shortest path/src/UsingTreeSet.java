...
public class UsingTreeSet {
    ...
    public static ArrayList<Integer> dijkstra(ArrayList<ArrayList<Integer>> vec, int vertices, int edges, int source) {
        ...
        // Create a TreeSet for Dijkstra's algorithm to efficiently extract minimum distance nodes
        TreeSet<Node> minDistanceSet = new TreeSet<>(new NodeComparator());
        distance[source] = 0;
        minDistanceSet.add(new Node(source, 0));

        while (!minDistanceSet.isEmpty()) {
            Node node = minDistanceSet.pollFirst();
            int u = node.vertex;
            ...
            for (Edge edge : adjList.get(u)) {
                ...
                if (distance[u] + weight < distance[v]) {
                    ...
                    minDistanceSet.add(new Node(v, distance[v]));
                }
            }
        }
    ...
    static class NodeComparator implements Comparator<Node> {
        public int compare(Node node1, Node node2) {
            int cmp = Integer.compare(node1.distance, node2.distance);
            if (cmp != 0) {
                return cmp;
            }
            return Integer.compare(node1.vertex, node2.vertex);
        }
    }
}
